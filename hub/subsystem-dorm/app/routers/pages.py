...
def _room_dict(r: Room) -> dict:
    return {"id": str(r.id), "room_number": r.room_number, "building": r.building,
...
def _resident_dict(r: Resident) -> dict:
    return {"id": str(r.id), "full_name": r.full_name, "email": r.email,
            "student_id": r.student_id, "faculty": r.faculty, "role_in_sub": r.role_in_sub,
            "status": r.status,
...
def _reservation_dict(res: Reservation) -> dict:
    return {"id": str(res.id), "room_id": str(res.room_id), "status": res.status,
            "reason": res.reason, "reject_reason": res.reject_reason,
...
def api_me(
    ...
    resident = db.query(Resident).filter(Resident.hub_user_id == user.hub_user_id).first()
    current_room = None
    ...
    return JSONResponse({
        "user": {"hub_user_id": user.hub_user_id, "email": user.email,
                 "full_name": user.full_name, "role_in_sub": user.role_in_sub,
                 "faculty": user.faculty, "student_id": user.student_id, "phone": user.phone},
        "resident": _resident_dict(resident) if resident else None,
        "current_room": current_room,
        "reservations": [{"reservation": _reservation_dict(r), "room": _room_dict(rm)}
...
def api_home(
    ...
    latest_reservation = (
        ...
        .filter(Reservation.hub_user_id == user.hub_user_id, Reservation.cancelled_at.is_(None))
        .order_by(Reservation.created_at.desc())
        .first()
    )
    ...
    total_residents = db.query(Resident).filter(Resident.status != "checked_out").count()
    checked_in_residents = db.query(Resident).filter(Resident.status == "checked_in").count()
    pending_count = db.query(Reservation).filter(
        Reservation.status == "pending", Reservation.cancelled_at.is_(None)
    ).count()
    ...
    return JSONResponse({
        ...
        "resident": _resident_dict(resident) if resident else None,
        "current_room": current_room,
        "latest_reservation": _reservation_dict(latest_reservation) if latest_reservation else None,
        "stats": {"total_rooms": total_rooms, "available_rooms": available_rooms,
                  "total_residents": total_residents, "checked_in_residents": checked_in_residents,
...
def api_rooms(
    ...
    occupancy = {}
    for room in rooms:
        occupancy[str(room.id)] = db.query(Resident).filter(
            Resident.room_id == room.id, Resident.status != "checked_out"
        ).count()
    return JSONResponse({
...
def api_room_detail(
    ...
    occupants = db.query(Resident).filter(
        Resident.room_id == room.id, Resident.status != "checked_out"
    ).all()
...
def api_teacher_overview(
    ...
    return JSONResponse({
        "user": {"full_name": user.full_name, "email": user.email,
                 "role_in_sub": user.role_in_sub, "faculty": user.faculty},
        "stats": {"total_rooms": total_rooms, "available_rooms": available_rooms,
                  "total_residents": total_residents, "checked_in": checked_in,
                  "occupancy_pct": round(checked_in / total_rooms * 100) if total_rooms else 0},
    })
...
def api_teacher_residents(
    ...
    return JSONResponse({
        "rows": [{"resident": _resident_dict(r), "room": _room_dict(rm) if rm else None}
                 for r, rm in rows],
    })
...
def home(
    ...
    """หน้าหลัก — redirect ไป React SPA."""
    if user is None:
