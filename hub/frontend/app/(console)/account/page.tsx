...
/**
 * บัญชีของฉัน (Account) — admin profile + Passkey / Backup codes management.
 * รวมข้อมูลโปรไฟล์ (จาก /api/me) เข้ากับการจัดการ Passkey (ย้ายมาจาก /account/security).
 */
      ...
      <div className="px-8 py-6 max-w-3xl space-y-6">
        ...
        <div className="bg-white rounded-xl border border-ink-200 p-6 shadow-sm">
          ...
          <dl className="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-5 pt-5 border-t border-ink-100">
            ...
            <Field
              label="สิทธิ์"
              value={me?.is_hub_admin ? "ผู้ดูแลระบบ (Hub Admin)" : me?.user_type ?? "—"}
            />
          </dl>
        </div>
