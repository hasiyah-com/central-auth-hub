...
**วันที่:** 26 ส.ค. 2026 · **แก้ไขล่าสุด:** 31 ส.ค. 2026 (ปิดช่องว่าง measured-vs-deployed)
...
> 🔴 **สำคัญ — ตัวเลขในเอกสารนี้เพิ่งจะตรงกับระบบที่รันจริงเมื่อ 31 ส.ค. 2026**
>
> การทดลองทุกชุดวัดด้วย `aggregate(rule, beh, **NEUTRAL**)` คือ **ตั้ง IForest 23 ฟีเจอร์
> เป็น 0** แต่ production เดิมส่งคะแนนจริงเข้าไป ซึ่ง `map_score()` บวกได้ถึง **+0.40**
> เทียบ threshold challenge 0.70 — วัดจากข้อมูลจริง 1,024 sessions พบว่า
> **128 ครั้ง (12.5%) ของการตัดสินถูกกำหนดโดยชั้นที่การทดลองวัดว่าไม่มีส่วนร่วม**
> (allow→warn 65 · warn→challenge 41 · **challenge→block 22**)
>
> แก้เมื่อ 31 ส.ค. 2026 โดยถอด IForest ออกจาก `aggregate()` (`monitoring_only()`) —
> **ตัวเลขการทดลองไม่ต้องแก้ ระบบที่รันเป็นฝ่ายขยับมาตรงกับตัวเลข** · ไม่มีการปรับโมเดลใดๆ
> รายละเอียด: [`l3_unified_2026-08-31.md`](../hub/backend/tests/reports/l3_unified_2026-08-31.md) · B66
...
> ```
>
> **L3 ไม่แตะ `access_decision` ทุกกรณี — ครอบคลุมทั้งสองมุมมองตั้งแต่ 31 ส.ค. 2026**
> (`sequence view` 18 มิติ แยกแกนตั้งแต่ 29 ส.ค. · `point view` 23 ฟีเจอร์ แยกเมื่อ 31 ส.ค.)
> รวมถึงไม่ทำ `allow → warn`
