...
**B67. SHAP ของ IsolationForest อิ่มตัวเมื่อจุดหลุด distribution ทุกมิติ — คำอธิบายดูดีแต่ไม่บอกอะไร**
- อาการ: window ที่ต่างกัน 6 แบบ (แต่ละแบบให้มิติคนละตัวเด่นสุด) ได้คะแนน **0.7439 เท่ากันเป๊ะทุกแบบ** และ SHAP ตอบ `gap_log_slope_w5` ทุกครั้ง
- สาเหตุ: จุดที่อยู่นอก distribution ในทุกมิติถูก IsolationForest isolate ที่ความลึกเท่ากันหมด → path length ไม่ต่างกัน → SHAP เหลือแค่สะท้อนว่าต้นไม้บังเอิญ split ฟีเจอร์ไหนก่อน ไม่ได้สะท้อนสาเหตุ · ในย่านที่**ยังไม่หลุด**ทุกมิติ SHAP ชี้ถูก 6/6
- **กฎ:** คำอธิบายของ anomaly detector เชื่อถือได้เฉพาะ**ในย่านที่คะแนนยังไล่ระดับตามความผิดปกติ** — ถ้าคะแนนอิ่มตัว คำอธิบายก็อิ่มตัวตาม · อย่ารายงาน top feature ของ SHAP เป็น "สาเหตุ" โดยไม่ตรวจว่าคะแนนอยู่ในย่านที่แยกแยะได้ · เทสที่พิสูจน์ว่า SHAP ชี้ถูกต้องทำในย่านที่แยกแยะได้ ไม่ใช่ย่านอิ่มตัว (ไม่งั้นได้ผลว่า "ถูก 1/6" แล้วสรุปผิดว่า mapping พัง)
- **สถานะ:** ทราบแล้ว ยังไม่แก้ · แนวทาง: `feature_perturbation="interventional"` + background dataset รายคน หรือรายงาน z-score รายมิติควบคู่
- **Verify:** `tests/test_l3_unified.py::test_shap_names_point_at_the_right_dimension` (6/6 ในย่านแยกแยะได้) · รายละเอียดเต็ม `tests/reports/l3_unified_2026-08-31.md` §2.1
