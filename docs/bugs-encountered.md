...
## 🧪 Measurement Integrity / Redaction (Week 12-13)

**B64. `git filter-repo --replace-text` ไม่แตะไฟล์ที่บีบอัด — `.docx`/`.pptx`/`.xlsx` คือ ZIP**
- อาการ: rewrite ประวัติทั้ง repo เพื่อลบ PII แล้ว scanner รายงาน "สะอาด" แต่เปิดไฟล์ `.docx` ในประวัติยังเจอชื่อจริง
- สาเหตุ: `--replace-text` ทำงานกับ **byte stream ของ blob ตรงๆ** ไฟล์ Office เป็น ZIP → ข้อความจริงถูก deflate ไว้ ไม่มี pattern ไหน match ได้ · ไฟล์ยังผ่านทุกกฎเพราะ "ไม่มีข้อความที่ค้นหาอยู่ในรูปแบบที่อ่านได้"
- **กฎ:** ไฟล์บีบอัด (`.docx .pptx .xlsx .zip .gz` และไฟล์ไบนารีทุกชนิด) **redact ด้วยการแทนข้อความไม่ได้** — ต้อง `--invert-paths` ลบทั้ง path แล้ว commit ไฟล์ฉบับสะอาดกลับเข้าไปใหม่ · scanner ต้อง unzip ไฟล์ Office ก่อนสแกน ไม่ใช่สแกน byte ดิบ
- **Verify:** `scripts/scan_history_pii.py` unzip ไฟล์ Office ก่อนตรวจทุกครั้ง

**B65. redactor กับ scanner ใช้แหล่งข้อมูลเดียวกัน = จุดบอดเดียวกัน ("ตรวจไม่เจอ" ≠ "สะอาด")**
- อาการ: scanner รายงาน 0 findings อย่างมั่นใจ ทั้งที่ `git grep` หาเจอด้วยตาเปล่า — เกิดสองครั้งจากคนละสาเหตุ: (1) กฎ host มี byte `` แฝงจาก heredoc ที่เขียนสคริปต์ ทำให้ regex ไม่เคย match เลย (2) roster ครอบคลุมแค่ 5 จาก 12 อีเมลที่อยู่ในประวัติจริง
- สาเหตุเชิงโครงสร้าง: เครื่องมือ**ล้าง**กับเครื่องมือ**ตรวจ** อ่าน mapping ชุดเดียวกัน → อะไรที่ mapping ไม่รู้จัก ทั้งสองตัวก็มองไม่เห็นเหมือนกัน "ผ่าน" จึงไม่ได้แปลว่าสะอาด แปลว่าไม่มีอะไรนอกรายการถูกมองหา
- **กฎ:** (1) การยืนยันว่าสะอาดต้องมาจาก**วิธีที่เป็นอิสระจากตัวล้าง** (เช่น `git grep` โดเมนดิบ, regex อีเมลทั่วไปที่ไม่พึ่ง roster) (2) เครื่องมือตรวจความปลอดภัยต้องมี `--self-test` ที่ยืนยันว่า**ทุกกฎยังจับตัวอย่างที่รู้ว่าผิดได้อยู่** — กฎที่พังเงียบให้ผลลัพธ์เหมือนกฎที่ผ่าน (บทเรียนเดียวกับ B61) (3) heredoc ที่มี `
   ` เสี่ยงถูกตีความ — ตรวจ control byte ในไฟล์ที่เพิ่งสร้างเสมอ
- **Verify:** `python scripts/scan_history_pii.py --self-test`

**B66. 🔴 การทดลองวัดระบบที่ไม่ใช่ระบบที่ deploy — harness ตั้งชั้นหนึ่งเป็นศูนย์ แต่ production ไม่ได้ตั้ง**
- อาการ: ตัวเลข recall/FPR ที่ freeze ไว้ทั้งชุดใช้อ้างอิงไม่ได้ เพราะวัดจากคอนฟิกที่ production ไม่เคยรัน
- สาเหตุ: สคริปต์ทดลองทุกตัว (`exp_final_gate.py:204`, `exp_lc_v3.py:157`, `exp_campaign_level.py:93`) เรียก `aggregate(rule, beh, **NEUTRAL**)` คือตั้ง IForest = 0 ส่วน `risk_engine.py` เรียกด้วยคะแนนจริงที่ `map_score()` บวกได้ถึง **+0.40** เทียบ threshold challenge 0.70 · ไม่มี error ไม่มี warning — สองเส้นทางเรียกฟังก์ชันเดียวกันด้วยอาร์กิวเมนต์ต่างกันเฉยๆ
- **ผลที่วัดได้จากข้อมูลจริง (1,024 sessions):** **128 ครั้ง (12.5%) การตัดสินถูกกำหนดโดยชั้นที่การทดลองวัดว่าไม่มีส่วนร่วม** — allow→warn 65, warn→challenge 41, **challenge→block 22**
- **กฎ:** **harness ของการทดลองต้องเรียกเส้นทางเดียวกับ production หรือมีเทสที่พิสูจน์ว่าคอนฟิกตรงกัน** · ถ้าจงใจปิดชั้นใดตอนวัด ต้องปิดที่ production ด้วย หรือระบุในรายงานว่าตัวเลขนี้ไม่ใช่ระบบที่รันจริง · ตัวเลขประสิทธิภาพผูกกับ**คอนฟิก** ไม่ใช่แค่โมเดล
- **Fix:** `iforest_scorer.monitoring_only()` — `aggregate()` รับ `risk_score=0.0` เสมอ (คงค่าดิบใน `iforest_raw` + SHAP ไว้ครบ) ทำให้ production ตรงกับที่วัด · ผลพลอยได้: access decision ไม่ขึ้นกับ ml-service อีกต่อไป
- **Verify:** `tests/test_l3_access_monitoring_split.py::test_point_view_never_moves_access_decision` (สแกนคะแนน 0.0/0.35/0.55/0.75/0.99) + `::test_point_view_reaches_monitoring_axis` (ตัดอิทธิพลแล้วสัญญาณต้องไม่หาย)

**B67. คำอธิบายของ anomaly detector เสื่อม *ก่อน* คะแนนอิ่มตัว — พังพอดีในย่านที่ใช้งานจริง**
- อาการ: `tree_path_dependent` SHAP ของ L3 sequence ชี้มิติที่ผิดปกติได้ถูก 6/6 ตอนคะแนนต่ำ แต่พอคะแนน**ผ่านเกณฑ์แจ้งเตือน** ความแม่นร่วงเป็น 4/6 → 3/6 → 2/6 → 1/6 **ทั้งที่คะแนนยังแยกกันได้ครบ 6 ค่า**
- สาเหตุ: SHAP แบบนี้อธิบายจากเส้นทางที่ต้นไม้เดิน · เมื่อจุดอยู่นอกกลุ่มหลายมิติพร้อมกัน ต้นไม้ส่วนใหญ่ตัดจบตั้งแต่ปมแรกด้วยฟีเจอร์ที่ปมนั้นบังเอิญเลือกไว้ตอนเทรน (ล็อกด้วย `random_state`) → น้ำหนักคำอธิบายกลายเป็นคุณสมบัติของ**โครงสร้างโมเดล** ไม่ใช่ของ**ข้อมูลที่กำลังอธิบาย** · คะแนนยังขยับเพราะต้นไม้ส่วนน้อยที่เดินลึกกว่า แต่ attribution ถูกครอบโดยส่วนใหญ่แล้ว
- **มีเพดานคะแนนแยกอีกเรื่อง:** ผิดปกติ 8 เท่ากับ 100,000 เท่าได้คะแนนเท่ากันทุกบิต (`0.743853` สำหรับ fixture+คอนฟิกนี้ **ไม่ใช่ค่าสากล**) — แต่เพดานเกิด**ทีหลัง** จุดที่ SHAP เริ่มพัง
- **กฎ:** (1) อย่ารายงาน SHAP เป็น "สาเหตุ" หรือ "มิติที่เบี่ยงเบนมากที่สุด" — ตอบคำถามนั้นด้วย **robust deviation รายมิติ** `(x−median)/IQR` ซึ่งคำนวณตรงจากข้อมูล ไม่มีเพดาน ไม่ขึ้นกับโครงสร้างต้นไม้ (วัดได้ 6/6 ทุกย่านรวมย่านอิ่มตัว) (2) คำอธิบายต้องคำนวณบน**อินพุตจริงที่โมเดลใช้** (18 มิติ) ไม่ใช่ residual 6 ค่า ไม่งั้นอธิบายคนละสิ่งกับที่ตัดสิน (3) ชื่อฟิลด์ต้องไม่สื่อเกินหลักฐาน — `top_factors` → `diagnostic_factors` (ข้อมูล) + `model_attribution` (debug พร้อม caveat)
- **บทเรียนเชิงวิธีวิทยา:** รอบแรกสรุปว่า "SHAP พังเพราะคะแนนอิ่มตัว" จากการทดลอง **2 จุดปลายทาง** ซึ่งบังเอิญอยู่คนละฝั่งของทั้งเกณฑ์ยิงและเพดาน → เห็นเป็นเหตุ-ผลที่ไม่มีจริง และเสนอทางแก้ที่ใช้ไม่ได้ · **การอนุมานสาเหตุจากสองจุดปลาย ต้องมีจุดกลางมายืนยันเสมอ**
- **Verify:** `tests/test_l3_explainability.py` (19 tests · spike ครบ 6 มิติ × 7 ย่าน) · รายละเอียด `tests/reports/l3_explainability_2026-09-01.md`
...
**B81. ตัวประเมินวัดคนละหน่วยกับเกณฑ์ที่ประกาศไว้ — guard ของ campaign recall ใช้หน่วยเหตุการณ์แทนหน่วยแคมเปญ**
- อาการ: รายงาน recalibration แสดง `campaign_recall` ของ B = 0.7175 และ H = 0.6492 (ต่าง −0.0684) แต่ guard ที่ตัดสินกลับรายงาน paired delta = −0.0454 · ผู้ใช้ทักว่าตัวเลขไม่ตรงกัน
- สาเหตุ: เกณฑ์ §9.4 เขียนว่า "campaign recall" ซึ่งนิยามไว้ที่**หน่วยแคมเปญ** (จับได้ถ้ามีเหตุการณ์ใดถึง challenge) แต่โค้ดเรียก `_paired_delta(rows_h, rows_b, {"campaign"})` ซึ่งเป็น recall **ระดับเหตุการณ์**ของตระกูลชื่อ `campaign` · ทั้งสองค่าถูกต้องในตัวเอง แต่ guard ตัดสินด้วยหน่วยที่ไม่ตรงกับที่ประกาศ
- ผลกระทบรอบนี้: คำตัดสินไม่เปลี่ยน (หน่วยที่ถูกยิ่งแย่กว่า และ H ล้มจากข้ออื่นด้วย) แต่ถ้าค่าอยู่ใกล้เส้น การตัดสินอาจกลับด้านได้โดยไม่มีใครรู้
- **กฎ:** ชื่อของตัวชี้วัดในเกณฑ์ต้องผูกกับ**หน่วยนับ**เสมอ (per-event / per-campaign / per-user) · guard ที่ตัดสินต้องมีเทสที่แยกสองหน่วยออกจากกันด้วยตัวอย่างที่ให้ผลต่างกัน · ตัวชี้วัดที่มีชื่อซ้ำกันคนละหน่วย ต้องตั้งชื่อฟังก์ชันให้ต่างกัน (`paired_campaign_delta` เทียบ `_paired_delta(..., {"campaign"})`)
- **กฎเพิ่ม:** ตัวประเมินต้องเก็บ**ผลดิบระดับหน่วยนับ**ไว้ด้วย ไม่ใช่เก็บแต่ค่าสรุป — ไม่งั้นแก้การวิเคราะห์ภายหลังต้องรันวัดใหม่ทั้งรอบ (เจอจริงตอนจะคำนวณ CI ที่หน่วยถูกต้อง)
- **Verify:** `tests/test_accuracy_gate.py::test_campaign_paired_delta_uses_campaign_units_not_events` · `::test_campaign_paired_delta_detects_a_missed_campaign` · รายงาน `accuracy_gate_2026-09-23.md` §10.5
